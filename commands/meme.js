  ...
  if (
    ...
    if (!fs.existsSync(memeFolder + fileName)) {
      memefile[guildID][memeName].splice(
        memefile[guildID][memeName].indexOf(fileName),
        1
      );
      fs.writeFileSync(
        memeLog,
...
function postMeme(message, args, edgy, type) {
  ...
  if (args.length === 0 && !(memefile[guildID] === undefined)) {
    ...
    if (
      !fileExists(
        parseName(memefile[guildID][whichMeme][meme]),
        whichMeme,
        guildID,
        memefile,
        edgy
    ...
    if (memefile[guildID][memeName].length === 0) {
      message.channel.send(
        `alright, heres the deal. Somehow, the name for that meme exists but it doesn't have anything associated. you can add some if you'd like. Or complain to kelsey. Please, though, try to have some agency okay?`
      );
      return;
    }
    if (memefile[guildID][memeName].length > 1) {
      ...
      if (
        !fileExistsReg(
          parseName(memefile[guildID][memeName][meme]),
          memeName,
          guildID,
          memefile
        )
      ...
      if (
        !fileExistsReg(
          parseName(memefile[guildID][memeName][0]),
          memeName,
          guildID,
          memefile
        )
      ...
      message.channel.send(
        `alright, heres the deal. Somehow, the name for that ${type} exists but it doesn't have anything associated. you can add some if you'd like. Or complain to kelsey. Please, though, try to have some agency okay?`
      );
    }
  }
}
