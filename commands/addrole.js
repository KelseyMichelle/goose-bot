...
let rolePerms = {
    ...
    "-mod": {
        "permissions": ["MANAGE_ROLES", "BAN_MEMBERS", "KICK_MEMBERS", "MANAGE_NICKNAMES", "CHANGE_NICKNAME", "MOVE_MEMBERS","MENTION_EVERYONE", "MANAGE_EMOJIS", "ADD_REACTIONS", "MANAGE_MESSAGES"],
        "requirement": ["ADMINISTRATOR"],
        "mentionable": true,
    },
}
...
module.exports = {
    ...
    execute(message, args) {
        ...
        if (rolePerm in rolePerms) {
            if (access.has_permission.check_list(message, rolePerms[args[rolePerm]])) {
                    let roleData =  rolePerms[rolePerm];
                    let roleColor = args[1].startsWith("#") && args[1].length === 7 ? args[1] : "WHITE";
                    let roleName = "";
                    if (args[2] && args[0].startsWith('#')) {
                        roleName = args[1].length === 7 ? args.splice(2).join(" ") : args.splice(1).join(" ");
                    ...
                    message.guild.roles.create({
                        ...
                        message.channel.send("you do not have permission to make roles in this server");
                    }
            } else {
